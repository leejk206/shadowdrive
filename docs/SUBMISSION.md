...
- [ ] 좌드래그(아크볼 회전) / shift+좌드래그(이동) / 우드래그·휠(카메라) / space(Go) / r(reset) 인터랙션 정상.
...
- **Compound shape 분해**: L/T/notch 실루엣을 회전 가능한 bar parts로 분해(`shapes.expandCompound`), 상태머신이 오클루더 회전(아크볼 쿼터니언)에 맞춰 각 part의 posRel·정점을 강체로 합성 — `src/core/shapes.js`, `src/core/GameStateMachine.js`.
- **인터랙션 컨트롤러**: 가동 오클루더 picking / 이동 / 아크볼 회전(쿼터니언 단일 진실원) — `src/ui/InteractionController.js`.
