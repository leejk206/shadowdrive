...
- **GO**: 3·2·1 카운트다운 → 오브젝트 본체가 사라지고 그림자만 남음 → 자동차 출발. 카운트다운 전까지는 물리가 적용되지 않고 차는 높은 출발점에 정지해 있다.
- **출발**: 시작 지점엔 발판이 없다. 차는 높은 출발점에서 **전진하며 낙하**해, 플레이어가 만든 그림자 도로에 착지해야 한다. 시작 지점 좁은 구간엔 그림자 도로가 생기지 않는다(마스킹).
